					...
					$(document).ready(function()
						{
							...
							$("#' . $idimg . '").on(\'click\', function(even) {
								...
								$("#' . $iddiv . '").load(\'minuteur.php\',{ name:"' . $name . '", pin:' . $pin . ', label:"' . $label . '", state:' . $nextstate . '});
							});			
						});
					</script>
				</div>
...
if (isset($_POST['name']) and isset($_POST['pin']) and isset($_POST['state']) and isset($_POST['tempo']) and isset($_POST['label']))
{
	...
	{
		throw new \GpioException('Minuteur Exception => ' . $e);
	}
